...
exports.combine = function (config) {
  ...
  return function (req, res, next) {
    ...
    function finish() {
      ...
      /** Minify with uglify */
      if (minify && type === MIME_TYPES['.js']){
        var ast = jsp.parse(res.body); // parse code and get the initial AST
        ast = pro.ast_mangle(ast); // get a new AST with mangled names
        ast = pro.ast_squeeze(ast); // get an AST with compression optimizations
        res.body = pro.gen_code(ast); // compressed code here    
      }
    ...
    query.forEach(function (relativePath, i) {
      ...
      fs.realpath(path.normalize(path.join(rootPath, relativePath)), function (err, absolutePath) {
        ...
        if (err || !absolutePath /* really bad idea but..|| absolutePath.indexOf(rootPath) !== 0 */) {
          return next(new BadRequest);
        }
